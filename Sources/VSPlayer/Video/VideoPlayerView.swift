...
open class VideoPlayerView: PlayerView {
    ...
    public private(set) var resource: VSPlayerResource? {
        didSet {
    ...
    override public var playerLayer: VSPlayerLayer? {
        didSet {
...
//        }
    ...
    open func setupUIComponents() {
        ...
        topMaskView.isHidden = VSOptions.topBarShowInCase != .always
    ...
    override open func player(layer: VSPlayerLayer, currentTime: TimeInterval, totalTime: TimeInterval) {
    ...
    override open func player(layer: VSPlayerLayer, state: VSPlayerState) {
    ...
    open func set(resource: VSPlayerResource, definitionIndex: Int = 0, isSetUrl: Bool = true) {
        currentDefinition = definitionIndex >= resource.definitions.count ? resource.definitions.count - 1 : definitionIndex
        if isSetUrl {
            let asset = resource.definitions[currentDefinition]
            super.set(url: asset.url, options: asset.options)
    ...
    override open func set(url: URL, options: VSOptions) {
        set(resource: VSPlayerResource(url: url, options: options))
    }
    ...
    open func panGestureChanged(velocity point: CGPoint, direction: VSPanDirection) {
        if direction == .horizontal {
            if !VSOptions.enablePlaytimeGestures {
                return
            }
            isSliderSliding = true
            if totalTime > 0 {
    ...
    open func panValue(velocity point: CGPoint, direction: VSPanDirection, currentTime _: Float, totalTime: Float) -> Float {
        if direction == .horizontal {
