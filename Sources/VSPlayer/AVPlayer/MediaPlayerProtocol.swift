...
public protocol MediaPlayerProtocol: MediaPlayback {
    ...
    @available(tvOS 14.0, *)
    var pipController: VSPictureInPictureController? { get }
    var dynamicInfo: DynamicInfo? { get }
    init(url: URL, options: VSOptions)
    func replace(url: URL, options: VSOptions)
    func play()
    func pause()
...
public extension MediaPlayerTrack {
    ...
    var colorSpace: CGColorSpace? {
        VSOptions.colorSpace(ycbcrMatrix: yCbCrMatrix as CFString?, transferFunction: transferFunction as CFString?)
    }
...
func setHttpProxy() {
    guard VSOptions.useSystemHTTPProxy else {
        return
    }
