...
class SyncPlayerItemTrack<Frame: MEFrame>: PlayerItemTrackProtocol, CustomStringConvertible {
    var seekTime = 0.0
    fileprivate let options: VSOptions
    ...
    required init(mediaType: AVFoundation.AVMediaType, frameCapacity: UInt8, options: VSOptions) {
...
//                    VSLog("seektime \(self.seekTime), frame \(frame.seconds), mediaType \(packet.assetTrack.mediaType)")
                ...
                VSLog("Decoder did Failed : \(error)")
                if decoder is VideoToolboxDecode {
                    ...
                    VSLog("VideoCodec switch to software decompression")
...
final class AsyncPlayerItemTrack<Frame: MEFrame>: SyncPlayerItemTrack<Frame> {
    ...
    required init(mediaType: AVFoundation.AVMediaType, frameCapacity: UInt8, options: VSOptions) {
        ...
        operationQueue.name = "VSPlayer_" + mediaType.rawValue
