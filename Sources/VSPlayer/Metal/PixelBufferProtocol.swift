...
extension CVPixelBuffer: PixelBufferProtocol {
    ...
    public var formatDescription: CMVideoFormatDescription? {
        ...
        if err != noErr {
            VSLog("Error at CMVideoFormatDescriptionCreateForImageBuffer \(err)")
        }
...
class PixelBuffer: PixelBufferProtocol {
    ...
    init(frame: AVFrame) {
        ...
        colorspace = VSOptions.colorSpace(ycbcrMatrix: yCbCrMatrix, transferFunction: transferFunction)
        ...
        formats = VSOptions.pixelFormat(planeCount: planeCount, bitDepth: bitDepth)
        ...
        for i in 0 ..< planeCount {
            ...
            if byteCount == size {
                ...
                let source = bytes[i]!
                var j = 0
                // Performance: while > stride(from:to:by:) > for in
                while j < height {
