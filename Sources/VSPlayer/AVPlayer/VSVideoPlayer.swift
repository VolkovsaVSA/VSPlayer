...
public struct VSVideoPlayer {
    public private(set) var coordinator: Coordinator
    public let url: URL
    public let options: VSOptions
    public init(coordinator: Coordinator, url: URL, options: VSOptions) {
...
extension VSVideoPlayer: UIViewRepresentable {
    ...
    public final class Coordinator: ObservableObject {
        public var state: VSPlayerState {
        ...
        public var onFinish: ((VSPlayerLayer, Error?) -> Void)?
        public var onStateChanged: ((VSPlayerLayer, VSPlayerState) -> Void)?
        ...
        public func makeView(url: URL, options: VSOptions) -> UIView {
            defer {
            ...
            if let playerLayer {
                ...
                let playerLayer = VSPlayerLayer(url: url, options: options, delegate: self)
        ...
        public func mask(show: Bool, autoHide: Bool = true) {
            ...
            if show {
                ...
                if autoHide {
                    ...
                    DispatchQueue.main.asyncAfter(deadline: DispatchTime.now() + VSOptions.animateDelayTimeInterval,
                                                  execute: delayHide!)
                }
            }
            #if os(macOS)
...
extension VSVideoPlayer.Coordinator: VSPlayerLayerDelegate {
    public func player(layer: VSPlayerLayer, state: VSPlayerState) {
    ...
    public func player(layer _: VSPlayerLayer, currentTime: TimeInterval, totalTime: TimeInterval) {
    ...
    public func player(layer: VSPlayerLayer, finish error: Error?) {
        onFinish?(layer, error)
    }

    public func player(layer _: VSPlayerLayer, bufferedCount: Int, consumeTime: TimeInterval) {
        onBufferChanged?(bufferedCount, consumeTime)
    }
}

extension VSVideoPlayer: Equatable {
    public static func == (lhs: VSVideoPlayer, rhs: VSVideoPlayer) -> Bool {
        lhs.url == rhs.url
    }
}

@MainActor
public extension VSVideoPlayer {
    ...
    /// Playing to the end.
    func onFinish(_ handler: @escaping (VSPlayerLayer, Error?) -> Void) -> Self {
    ...
    func onStateChanged(_ handler: @escaping (VSPlayerLayer, VSPlayerState) -> Void) -> Self {
