...
public protocol VSSubtitleProtocol {
    func search(for time: TimeInterval) -> [SubtitlePart]
}

public protocol SubtitleInfo: VSSubtitleProtocol, AnyObject, Hashable, Identifiable {
...
public class VSSubtitle {
    public var parts: [SubtitlePart] = []
    public init() {}
}

extension VSSubtitle: VSSubtitleProtocol {
...
public extension VSSubtitle {
    ...
    func parse(data: Data, encoding: String.Encoding? = nil) throws {
        ...
        let parse = VSOptions.subtitleParses.first { $0.canParse(scanner: scanner) }
        if let parse {
...
//    public static func == (lhs: VSURLSubtitle, rhs: VSURLSubtitle) -> Bool {
//        lhs.url == rhs.url
//    }
}
...
open class SubtitleModel: ObservableObject {
    ...
    private var subtitleDataSouces: [SubtitleDataSouce] = VSOptions.subtitleDataSouces
    @Published
