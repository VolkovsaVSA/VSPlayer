...
public final class MEPlayerItem: Sendable {
    ...
    private static var onceInitial: Void = {
        ...
        av_log_set_callback { ptr, level, format, args in
            ...
            if let ptr {
                ...
                if avclass == avfilter_get_class() {
                    ...
                    if let opaque = context.graph?.pointee.opaque {
                        let options = Unmanaged<VSOptions>.fromOpaque(opaque).takeUnretainedValue()
            ...
            VSLog(level: LogLevel(rawValue: level) ?? .warning, log)
        }
    }()

    weak var delegate: MEPlayerDelegate?
    public init(url: URL, options: VSOptions) {
        self.url = url
        self.options = options
        timer.fireDate = Date.distantFuture
        operationQueue.name = "VSPlayer_" + String(describing: self).components(separatedBy: ".").last!
...
//        }
    ...
    func startRecord(url: URL) {
        ...
        guard let outputFormatCtx, let formatCtx else {
            VSLog(NSError(errorCode: .formatOutputCreate, avErrorCode: ret))
            return
        }
        var index = 0
        var audioIndex: Int?
        ...
        guard ret >= 0 else {
            VSLog(NSError(errorCode: .formatWriteHeader, avErrorCode: ret))
    ...
    private func read() {
        readOperation = BlockOperation { [weak self] in
            ...
            Thread.current.stackSize = VSOptions.stackSize
            self.readThread()
        }
    ...
    private func readThread() {
        if state == .opened {
            if options.startPlayTime > 0 {
                ...
                VSLog("start PlayTime: \(timestamp.seconds) spend Time: \(CACurrentMediaTime() - seekStartTime)")
            }
            state = .reading
        }
...
//                var result = av_seek_frame(formatCtx, -1, timeStamp, seekFlags)
                ...
                if result < 0, seekFlags & AVSEEK_FLAG_BACKWARD == AVSEEK_FLAG_BACKWARD {
                    VSLog("seek to \(seekToTime) failed. seekFlags remove BACKWARD")
                    options.seekFlags &= ~AVSEEK_FLAG_BACKWARD
                    seekFlags &= ~AVSEEK_FLAG_BACKWARD
                    result = avformat_seek_file(formatCtx, -1, seekMin, timeStamp, seekMax, seekFlags)
                }
                VSLog("seek to \(seekToTime) spend Time: \(CACurrentMediaTime() - seekStartTime)")
                if state == .closed {
                    break
                }
        ...
        if readResult == 0 {
            if let outputFormatCtx, let formatCtx {
                   ...
                   let outputPacket
                {
                    ...
                    if ret < 0 {
                        VSLog("can not av_interleaved_write_frame")
                    }
                }
            }
