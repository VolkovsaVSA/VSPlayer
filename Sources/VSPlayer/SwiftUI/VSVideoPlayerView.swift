...
@MainActor
public struct VSVideoPlayerView: View {
    private let subtitleDataSouce: SubtitleDataSouce?
    @State
    private var title: String
    @StateObject
    private var playerCoordinator: VSVideoPlayer.Coordinator
    @Environment(\.dismiss)
    private var dismiss
    @FocusState
    ...
    public init(url: URL, options: VSOptions, title: String? = nil) {
        self.init(coordinator: VSVideoPlayer.Coordinator(), url: url, options: options, title: title, subtitleDataSouce: nil)
    }

    public init(coordinator: VSVideoPlayer.Coordinator, url: URL, options: VSOptions, title: String? = nil, subtitleDataSouce: SubtitleDataSouce? = nil) {
    ...
    public init(coordinator: VSVideoPlayer.Coordinator, url: State<URL>, options: VSOptions, title: State<String>, subtitleDataSouce: SubtitleDataSouce?) {
        _url = url
    ...
    private var playView: some View {
        VSVideoPlayer(coordinator: playerCoordinator, url: url, options: options)
    ...
    private func ornamentView(playerWidth: Double) -> some View {
        VStack(alignment: .leading) {
            VSVideoPlayerViewBuilder.titleView(title: title, config: playerCoordinator)
    ...
    private func ornamentControlsView(playerWidth _: Double) -> some View {
        HStack {
            VSVideoPlayerViewBuilder.playbackControlView(config: playerCoordinator, spacing: 16)
            Spacer()
            VideoTimeShowView(config: playerCoordinator, model: playerCoordinator.timemodel, timeFont: .title3.monospacedDigit())
            Spacer()
            Group {
                VSVideoPlayerViewBuilder.contentModeButton(config: playerCoordinator)
                VSVideoPlayerViewBuilder.subtitleButton(config: playerCoordinator)
                VSVideoPlayerViewBuilder.playbackRateButton(playbackRate: $playerCoordinator.playbackRate)
                VSVideoPlayerViewBuilder.infoButton(showVideoSetting: $showVideoSetting)
            }
            .font(.largeTitle)
        }
    }
...
struct VideoControllerView: View {
    @ObservedObject
    fileprivate var config: VSVideoPlayer.Coordinator
    @ObservedObject
    ...
    public var body: some View {
        VStack {
            ...
            Spacer()
            #if !os(xrOS)
            VSVideoPlayerViewBuilder.playbackControlView(config: config)
            Spacer()
            HStack {
                VSVideoPlayerViewBuilder.titleView(title: title, config: config)
                Spacer()
                playbackRateButton
                pipButton
                infoButton
    ...
    private var muteButton: some View {
        ...
        HStack {
            Slider(value: $config.playbackVolume, in: 0 ... 1)
                ...
                .padding(.leading, 16)
            VSVideoPlayerViewBuilder.muteButton(config: config)
        }
        .padding(16)
        .glassBackgroundEffect()
        #else
    ...
    private var contentModeButton: some View {
        VSVideoPlayerViewBuilder.contentModeButton(config: config)
    }
    ...
    private var subtitleButton: some View {
        VSVideoPlayerViewBuilder.subtitleButton(config: config)
    }

    private var playbackRateButton: some View {
        VSVideoPlayerViewBuilder.playbackRateButton(playbackRate: $config.playbackRate)
    }
...
struct VSVideoPlayerView_Previews: PreviewProvider {
    static var previews: some View {
        let url = URL(string: "http://clips.vorwaerts-gmbh.de/big_buck_bunny.mp4")!
        VSVideoPlayerView(coordinator: VSVideoPlayer.Coordinator(), url: url, options: VSOptions())
    }
}
