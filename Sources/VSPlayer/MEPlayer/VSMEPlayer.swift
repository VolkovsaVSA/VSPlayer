...
public class VSMEPlayer: NSObject {
    ...
    private lazy var _pipController: Any? = {
        if #available(iOS 15.0, tvOS 15.0, macOS 12.0, *), let videoOutput {
            ...
            let pip = VSPictureInPictureController(contentSource: contentSource)
            return pip
        } else {
            return nil
        }
    ...
    @available(tvOS 14.0, *)
    public var pipController: VSPictureInPictureController? {
        _pipController as? VSPictureInPictureController
    }
    ...
    public required init(url: URL, options: VSOptions) {
        VSOptions.setAudioSession()
        audioOutput = VSOptions.audioPlayerType.init()
        playerItem = MEPlayerItem(url: url, options: options)
        if options.videoDisable {
            videoOutput = nil
        } else {
            videoOutput = VSOptions.videoPlayerType.init(options: options)
        }
...
private extension VSMEPlayer {
    ...
    @objc private func spatialCapabilityChange(notification _: Notification) {
        VSLog("[audio] spatialCapabilityChange")
...
extension VSMEPlayer: MEPlayerDelegate {
    func sourceDidOpened() {
        ...
        let enabledAudioTrack = options.audioDisable ? nil : tracks(mediaType: .audio).first { $0.isEnabled } as? FFmpegAssetTrack
        let audioDescriptor = enabledAudioTrack?.audioDescriptor
        let hasValidCodecParameters = enabledAudioTrack.map {
            AudioDescriptor.hasDecodableCodecParameters($0.codecpar)
        } ?? false
        let hasRenderableFormat = enabledAudioTrack?.formatDescription != nil
        if audioDescriptor != nil, !audioDescriptor!.isDecodable || !hasValidCodecParameters || !hasRenderableFormat {
            VSLog("[audio] malformed audio track, falling back to video-only playback")
            playerItem.disableEnabledAudioTrack()
        }
        let shouldPrepareAudio = audioDescriptor?.isDecodable == true && hasValidCodecParameters && hasRenderableFormat
        runOnMainThread { [weak self] in
            guard let self else { return }
            if let audioDescriptor, shouldPrepareAudio {
    ...
    func sourceDidChange(oldBitRate: Int64, newBitrate: Int64) {
        VSLog("oldBitRate \(oldBitRate) change to newBitrate \(newBitrate)")
    }
}

extension VSMEPlayer: MediaPlayerProtocol {
    ...
    public var naturalSize: CGSize {
        options.display == .plane ? playerItem.naturalSize : VSOptions.sceneSize
    }
    ...
    public func replace(url: URL, options: VSOptions) {
        VSLog("replaceUrl \(self)")
        shutdown()
    ...
    public func shutdown() {
        ...
        if VSOptions.isClearVideoWhereReplace {
            videoOutput?.flush()
        }
    }
...
@available(tvOS 14.0, *)
extension VSMEPlayer: AVPictureInPictureSampleBufferPlaybackDelegate {
...
@available(macOS 12.0, iOS 15.0, tvOS 15.0, *)
extension VSMEPlayer: AVPlaybackCoordinatorPlaybackControlDelegate {
...
extension VSMEPlayer: DisplayLayerDelegate {
