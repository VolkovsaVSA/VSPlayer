...
open class IOSVideoPlayerView: VideoPlayerView {
    ...
    open func updateUI(isFullScreen: Bool) {
        ...
        if isFullScreen {
            ...
            viewController.present(fullVC, animated: true) {
                VSOptions.supportedInterfaceOrientations = fullVC.supportedInterfaceOrientations
            }
        } else {
            ...
            if let originalOrientations {
                VSOptions.supportedInterfaceOrientations = originalOrientations
            }
    ...
    open func updateUI(isLandscape: Bool) {
        if isLandscape {
            topMaskView.isHidden = VSOptions.topBarShowInCase == .none
        } else {
    ...
    override open func set(resource: VSPlayerResource, definitionIndex: Int = 0, isSetUrl: Bool = true) {
    ...
    override open func panGestureBegan(location point: CGPoint, direction: VSPanDirection) {
    ...
    override open func panGestureChanged(velocity point: CGPoint, direction: VSPanDirection) {
        if direction == .vertical {
            if isVolume {
                if VSOptions.enableVolumeGestures {
            ...
            } else if VSOptions.enableBrightnessGestures {
                #if !os(xrOS)
