...
class MEFilter {
    ...
    public init(timebase: Timebase, isAudio: Bool, nominalFrameRate: Float, options: VSOptions) {
...
//                    bufferSinkContext?.pointee.hw_device_ctx = framesCtxData.pointee.device_ref
    ...
    public func filter(options: VSOptions, inputFrame: UnsafeMutablePointer<AVFrame>, completionHandler: (UnsafeMutablePointer<AVFrame>) -> Void) {
        let filters: String
        if isAudio {
...
//                timebase = Timebase(av_buffersink_get_time_base(bufferSinkContext))
            completionHandler(inputFrame)
            // av_frame_unref must be called, otherwise memory will leak.
