...
public class VSSlider: UXSlider {
    ...
    @objc private func progressSliderTouchBegan(_ sender: VSSlider) {
    ...
    @objc private func progressSliderValueChanged(_ sender: VSSlider) {
        guard isPlayable else { return }
        delegate?.slider(value: Double(sender.value), event: .valueChanged)
    }

    @objc private func progressSliderTouchEnded(_ sender: VSSlider) {
        guard isPlayable else { return }
