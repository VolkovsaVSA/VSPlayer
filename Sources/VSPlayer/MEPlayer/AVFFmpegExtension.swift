...
extension AVCodecParameters {
    mutating func createContext(options: VSOptions?) throws -> UnsafeMutablePointer<AVCodecContext> {
...
extension AVChannelLayout: CustomStringConvertible {
    ...
    var layoutTag: AudioChannelLayoutTag? {
        VSLog("[audio] FFmepg AVChannelLayout: \(self) order: \(order) mask: \(u.mask)")
        ...
        if let tag {
            ...
            VSLog("[audio] can not find AudioChannelLayoutTag FFmepg channelLayout: \(self) order: \(order) mask: \(u.mask)")
            return nil
        }
    }
...
public extension NSError {
    convenience init(errorCode: VSPlayerErrorCode, avErrorCode: Int32) {
