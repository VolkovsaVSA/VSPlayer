...
extension AudioUnit {
    var channelLayout: UnsafeMutablePointer<AudioChannelLayout> {
        ...
        VSLog("[audio] unit tag: \(tag)")
        if tag == kAudioChannelLayoutTag_UseChannelDescriptions {
            VSLog("[audio] unit channelDescriptions: \(layout.channelDescriptions)")
            return layout
        }
...
extension AVAudioChannelLayout {
    func channelLayout() -> AVChannelLayout {
        VSLog("[audio] channelLayout: \(layout.pointee.description)")
        var mask: UInt64?
        if layoutTag == kAudioChannelLayoutTag_UseChannelDescriptions {
            ...
            for description in layout.channelDescriptions {
                ...
                VSLog("[audio] label: \(label)")
                let channel = label.avChannel.rawValue
                VSLog("[audio] avChannel: \(channel)")
                if channel >= 0 {
                    newMask |= 1 << channel
                }
            }
        ...
        VSLog("[audio] out mask: \(outChannel.u.mask) nb_channels: \(outChannel.nb_channels)")
        return outChannel
    }
