...
//    private let delay = AVAudioUnitDelay()
    ...
    public func prepare(audioFormat: AVAudioFormat) {
        ...
        VSLog("[audio] set preferredOutputNumberOfChannels: \(audioFormat.channelCount)")
        #endif
        VSLog("[audio] outputFormat AudioFormat: \(audioFormat)")
        if let channelLayout = audioFormat.channelLayout {
            VSLog("[audio] outputFormat channelLayout \(channelLayout.channelDescriptions)")
        }
        ...
        VSLog("[audio] new sourceNode inputFormat: \(sourceNode.inputFormat(forBus: 0))")
