 ...
 */
public enum VSPlayerState: CustomStringConvertible {
    case initialized
    case preparing
    case readyToPlay
    case buffering
...
@MainActor
public protocol VSPlayerLayerDelegate: AnyObject {
    func player(layer: VSPlayerLayer, state: VSPlayerState)
    func player(layer: VSPlayerLayer, currentTime: TimeInterval, totalTime: TimeInterval)
    func player(layer: VSPlayerLayer, finish error: Error?)
    func player(layer: VSPlayerLayer, bufferedCount: Int, consumeTime: TimeInterval)
}

open class VSPlayerLayer: NSObject {
    public weak var delegate: VSPlayerLayerDelegate?
    @Published
    ...
    public private(set) var options: VSOptions

    public var player: MediaPlayerProtocol {
        didSet {
            VSLog("player is \(player)")
            state = .initialized
    ...
    public private(set) var url: URL {
        didSet {
            ...
            if isWirelessRouteActive {
                ...
                firstPlayerType = VSOptions.firstPlayerType
            }
    ...
    public private(set) var state = VSPlayerState.initialized {
        willSet {
            if state != newValue {
                runOnMainThread { [weak self] in
                    guard let self else { return }
                    VSLog("playerStateDidChange - \(newValue)")
    ...
    public init(url: URL, isAutoPlay: Bool = VSOptions.isAutoPlay, options: VSOptions, delegate: VSPlayerLayerDelegate? = nil) {
        self.url = url
    ...
    public func set(url: URL, options: VSOptions) {
        self.options = options
        runOnMainThread {
            self.url = url
        }
    ...
    public func set(urls: [URL], options: VSOptions) {
        self.options = options
        self.urls.removeAll()
    ...
    open func play() {
        ...
        if #available(tvOS 14.0, *) {
            VSPictureInPictureController.mute()
        }
    }
...
extension VSPlayerLayer: MediaPlayerDelegate {
    ...
    public func finish(player: some MediaPlayerProtocol, error: Error?) {
        if let error {
            if type(of: player) != VSOptions.secondPlayerType, let secondPlayerType = VSOptions.secondPlayerType {
                self.player = secondPlayerType.init(url: url, options: options)
                return
            }
            state = .error
            VSLog(error as CustomStringConvertible)
        } else {
...
@available(tvOS 14.0, *)
extension VSPlayerLayer: AVPictureInPictureControllerDelegate {
...
extension VSPlayerLayer {
    ...
    @objc private func enterBackground() {
        ...
        if VSOptions.canBackgroundPlay {
