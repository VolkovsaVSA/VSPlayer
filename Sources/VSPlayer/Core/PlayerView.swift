...
public protocol PlayerControllerDelegate: AnyObject {
    func playerController(state: VSPlayerState)
...
open class PlayerView: UIView, VSPlayerLayerDelegate, VSSliderDelegate {
    ...
    public convenience init() {
        ...
        #else
        self.init(frame: CGRect(origin: .zero, size: VSOptions.sceneSize))
        #endif
    }
    ...
    open func seek(time: TimeInterval, completion: @escaping ((Bool) -> Void)) {
        playerLayer?.seek(time: time, autoPlay: VSOptions.isSeekedAutoPlay, completion: completion)
    }
    ...
    open func set(url: URL, options: VSOptions) {
        srtControl.url = url
        toolBar.currentTime = 0
        totalTime = 0
        playerLayer = VSPlayerLayer(url: url, options: options)
    }
    ...
    // MARK: - VSPlayerLayerDelegate

    open func player(layer: VSPlayerLayer, state: VSPlayerState) {
    ...
    open func player(layer _: VSPlayerLayer, currentTime: TimeInterval, totalTime: TimeInterval) {
    ...
    open func player(layer _: VSPlayerLayer, finish error: Error?) {
        delegate?.playerController(finish: error)
    }

    open func player(layer _: VSPlayerLayer, bufferedCount: Int, consumeTime: TimeInterval) {
